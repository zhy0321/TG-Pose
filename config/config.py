...
flags.DEFINE_string('dataset_dir', '/opt/data/private/zhy/Dataset/',
                    'path to the dataset')
flags.DEFINE_string('detection_dir',
                    '/opt/data/private/zhy/Dataset/detection_dir/segmentation_results/',
...
flags.DEFINE_float('aug_nl_pro', 0.3, 'non-linear augmentation')
...
flags.DEFINE_string("train_stage", 'TDA', "for train stage")
flags.DEFINE_string("run_stage", 'RL', "for run mode")
flags.DEFINE_string('device', 'cuda', '')
flags.DEFINE_integer("num_workers", 12, "cpu cores for loading dataset")
...
flags.DEFINE_float('h1_w', 4.0, '')
flags.DEFINE_float('h2_w', 4.0, '')
flags.DEFINE_float('align_w', 8.0, '')
flags.DEFINE_float('recon_re_w', 2.0, '')
flags.DEFINE_float('feat_consist_w', 2.0, '')
...
flags.DEFINE_float('equi_rot_w', 1.0, 'equivariance loss')
flags.DEFINE_float('equi_tran_w', 1.0, 'equivariance loss')
flags.DEFINE_float('equi_size_w', 1.0, 'equivariance loss')
flags.DEFINE_float('otrh_w', 1.0, 'otrh loss')
flags.DEFINE_float('inv_w', 1.0, 'invariance loss')
flags.DEFINE_float('pose_w', 1.0, 'Pose loss')
flags.DEFINE_float('DCD_align', 1.0, 'DCD_align loss')
flags.DEFINE_float('h1_align', 2.0, 'h1_align loss')
flags.DEFINE_float('h2_align', 2.0, 'h2_align loss')

flags.DEFINE_float('TDA_ph', 1.0, 'TDA loss')
...
# save parameters
# model to save
flags.DEFINE_integer('save_every', 1, '')  # save models every 'save_every' epoch
...
flags.DEFINE_string('RL_model_path', '', 'path to the saved RL model')
...
# TopologyNet
flags.DEFINE_integer('output_channels', 2500, 'neighbor number for gcn')
flags.DEFINE_integer('ratio', 1, 'crop_ball_from_depth_image')
