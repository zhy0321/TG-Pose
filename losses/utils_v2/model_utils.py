...
def calc_dcd(x, gt):
    ...
    exp_dist1, exp_dist2 = torch.exp(-dist1 * 1000), torch.exp(-dist2 * 1000)

    count1 = torch.zeros_like(idx2)
    count1.scatter_add_(1, idx1.long(), torch.ones_like(idx1))
    weight1 = count1.gather(1, idx1.long()).float().detach() ** 1
    ...
    weight2 = count2.gather(1, idx2.long()).float().detach() ** 1
