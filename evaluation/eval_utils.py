    ...
    for progress, result in enumerate(tqdm(pred_results)):
        ...
        for cls_id in cls_ids:
            ...
            if use_matches_for_pose:
                ...
                cls_pred_sRT = cls_pred_sRT[iou_thres_pred_match > -1] if len(iou_thres_pred_match) > 0 else np.zeros(
                    (0, 4, 4))
            ...
            pose_cls_gt_match, pose_cls_pred_match = compute_RT_matches(RT_overlaps, cls_pred_class_ids,
                                                                        cls_gt_class_ids,
...
def draw_detections(img, out_dir, data_name, img_id, img_name, intrinsics, pred_sRT, pred_size, pred_class_ids,
