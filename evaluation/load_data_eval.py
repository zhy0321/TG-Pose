...
class PcJitter(object):
    ...
    def __call__(self, points):
        ...
        noise = np.random.normal(loc=0, scale=self.std, size=(points.shape[0], 3)).clip(-self.clip, self.clip)
...
class PoseDataset(data.Dataset):
        ...
        if self.per_obj in self.cat_names and self.per_obj != 'can' and self.source == 'Real':
            ...
            if os.path.exists(img_list_cache_filename):
                ...
                img_list_obj = [line.rstrip('\n') for line in
                                open(os.path.join(self.data_dir, img_list_cache_filename))]
            else:
    ...
    def __getitem__(self, index):
        ...
        for j in range(num_instance):
            ...
            mask_point = distances > (diagonal_length * 0.25)
