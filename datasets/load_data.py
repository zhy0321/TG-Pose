...
from datasets.compute_pd import compute_pd
from datasets.data_augmentation import PC_BasicAugment, PcJitter, PcRandomCrop, PcRandomCutout, \
    PcNormalize, pc_sampler, PcRandomDropout


class PoseDataset(data.Dataset):
    def __init__(self, source=None, mode='train', run_stage=None, data_dir=None,
        ...
        self.invaild_list = []
        self.mug_sym = mmengine.load(os.path.join(self.data_dir, 'Real/train/mug_handle.pkl'))

        print('{} images found.'.format(self.length))
        print('{} models loaded.'.format(len(self.models)))

        # augmentation class
        self.base_aug = PC_BasicAugment()
        self.operator_name = ['Jitter', 'RandomCutout', 'RandomCrop', 'RandomDropout']

        self.custom_aug_operator = [PcJitter(std=0.005, clip=0.05, p=0.6),
                                    PcRandomCutout(p=0.9, min_num_points=1024),
                                    PcRandomCrop(p=0.9, min_num_points=1024),
                                    PcRandomDropout(p=0.9, max_dropout_ratio=0.5),
                                    ]
    ...
    def __getitem__(self, index):
        ...
        nocs_scale = gts['scales'][idx]
        # fsnet scale (from model) scale residual
        fsnet_scale, mean_shape = self.get_fs_net_scale(self.id2cat_name[str(cat_id + 1)], model,
                                                        nocs_scale)
        fsnet_scale = fsnet_scale / 1000.0
        mean_shape = mean_shape / 1000.0
        rotation = gts['rotations'][idx]
        translation = gts['translations'][idx]
        # add noise to roi_mask
        roi_mask_def = defor_2D(roi_mask, rand_r=FLAGS.roi_mask_r, rand_pro=FLAGS.roi_mask_pro)

        pcl_in_ori = self._depth_to_pcl(roi_depth, out_camK, roi_coord_2d, roi_mask_def) / 1000.0
        pcl_in = pcl_in_ori.copy()

        min_values = np.min(pcl_in, axis=0)
        max_values = np.max(pcl_in, axis=0)
        ranges = max_values - min_values
        diagonal_length = np.sqrt(np.sum(ranges ** 2))
        center_index = np.array([25])
        center_point = pcl_in[center_index]
        distances = np.linalg.norm(pcl_in - center_point, axis=1)
        mask_point = distances > (diagonal_length * 0.15)
        pcl_in = pcl_in[mask_point]
        ...
        points_category_id = np.load(
            './obj_model/points_{}.npy'.format(
                self.cat_names[cat_id]))
        pdh1_category_id = np.load(
            './obj_model/pdh1_{}.npy'.format(
                self.cat_names[cat_id]))
        pdh2_category_id = np.load(
            './obj_model/pdh2_{}.npy'.format(
                self.cat_names[cat_id]))
        ...
        data_dict['pdh1_category'] = torch.as_tensor(pdh1_category_id.astype(np.float32)).contiguous()
        data_dict['pdh2_category'] = torch.as_tensor(pdh2_category_id.astype(np.float32)).contiguous()
        data_dict['points_category'] = torch.as_tensor(points_category_id.astype(np.float32)).contiguous()

        # point augmentation
        if FLAGS.train:
            PC, gt_R, gt_t, gt_s = self.base_aug(data_dict)

            PC = self._sample_points(PC.squeeze(0).numpy(), 2048)  # shape: (1024, 3)
            pcl_in = self._sample_points(PC, 1024)  # shape: (1024, 3)

            data_dict['pcl_in'] = torch.as_tensor(pcl_in.astype(np.float32)).contiguous()
            data_dict['rotation'] = gt_R.squeeze(0)
            data_dict['translation'] = gt_t.squeeze(0)
            data_dict['fsnet_scale'] = gt_s.squeeze(0)

            pdh1, pdh2 = compute_pd(pcl_in)
            data_dict['pdh1'] = pdh1.contiguous()
            data_dict['pdh2'] = pdh2.contiguous()
            idx = np.random.randint(0, len(self.custom_aug_operator))
            aug_pc = self.custom_aug_operator[idx](torch.as_tensor(PC.astype(np.float32)).contiguous())
            aug_pc = pc_sampler(aug_pc, 1024)
            data_dict['aug_pcl_in'] = aug_pc
            data_dict['aug_name'] = self.operator_name[idx]

        return data_dict

    def _create_pose(self, rotation, translation):
        pose = np.eye(4)
        pose[:3, :3] = rotation
    ...
    def crop_ball_from_pts(self, pcl_in, center, radius, num_points=1024):
        distance = torch.sqrt(((pcl_in - center) ** 2).sum(-1))  # [N]
        ...
        pcl = self._sample_points(pcl_in[idx], num_points)

        return pcl

    def _depth_sphere_to_pcl(self, rotation, translation, scale, pcl_in,
                             ratio=0.6):
        pcl_in = torch.from_numpy(pcl_in)
        center = translation
        center = torch.from_numpy(center)
        rotation = torch.from_numpy(rotation)
        radius = ratio * torch.norm(rotation @ scale)
        pcl = self.crop_ball_from_pts(pcl_in, center, radius, num_points=1024)
