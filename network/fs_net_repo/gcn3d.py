...
def get_nearest_index(target: "(bs, v1, 3)", source: "(bs, v2, 3)"):
    ...
    Return: (bs, v1, 1)
    """
    inner = torch.bmm(target, source.transpose(1, 2))
    s_norm_2 = torch.sum(source ** 2, dim=2)
    t_norm_2 = torch.sum(target ** 2, dim=2)
...
class HS_layer(nn.Module):
    ...
    def graph_conv(self, receptive_fields_norm, neighbor_index, feature_map,
        ...
        theta = receptive_fields_norm @ support_direction_norm
        theta = self.relu(theta)
        theta = theta.reshape(bs, vertice_num, neighbor_num, -1)

        feature_map = feature_map @ self.weights + self.bias
        feature_center = feature_map[:, :, :self.out_channel]
        feature_support = feature_map[:, :, self.out_channel:]
        ...
        activation_support = torch.mean(activation_support, dim=2)
...
def get_ORL_global(feature:'(bs, vertice_num, in_channel)', vertices: '(bs, vertice_num, 3)', 
    ...
    feature = indexing_neighbor_new(feature, neighbor_index)
